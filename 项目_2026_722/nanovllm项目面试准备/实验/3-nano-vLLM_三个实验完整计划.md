...
准备 8 条固定纯文本 Prompt，每条经过 Chat Template 后严格为 **16384 Token**，每个请求固定生成 **2048 Token**。并发数设为**8**，运行压缩关闭和压缩开启两种模式。
...
模型初始化后先执行一次短预热，不计入结果。正式测试采用交错顺序：
...
| 零抢占最高并发 | 在 8 中完成率 100% 且无抢占的最大值 | 待测 |
